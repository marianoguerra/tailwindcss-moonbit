...
# The full correctness gate: fmt-check, check, test, diff.
[group('test')]
gate: fmt-check check test diff

# ---------------------------------------------------------------------------
# Build
# ---------------------------------------------------------------------------

# Build the native CLI to _build/native/release/build/cmd/tailwindcss/tailwindcss.exe
[group('build')]
build-cli:
...
profile-alloc workload="many":
    ...
    moon-pprof memprofile-native \
        _build/native/release/build/benchmarks/bench/bench.exe \
        -- --emit --reqfile benchmarks/workloads/{{workload}}/request.json
...
version:
    ...
    if [ "$mod" != "$pkg" ]; then echo "MISMATCH"; exit 1; fi

# The version lives in two files that must stay in sync.

# Bump the version in moon.mod and ffi/js/package.json.
[group('release')]
